...
func runMinimalSpecTestCases(t *testing.T, s *SszMinimalTest) {
	for _, testCase := range s.TestCases {
		...
		if !isEmpty(testCase.BeaconBlockBody.Value) {
			fmt.Println("Block body running...")
			fmt.Printf("%#x\n", testCase.BeaconBlockBody.Value.RandaoReveal)
			bb := testCase.BeaconBlockBody.Value
			fmt.Println(len(bb.ProposerSlashings))
			fmt.Println(len(bb.AttesterSlashings))
			fmt.Println(len(bb.Attestations))
			fmt.Println(len(bb.Deposits))
			fmt.Println(len(bb.VoluntaryExits))
			fmt.Println(len(bb.Transfers))
			compareSSZEncoding(t, &comparisonConfig{
